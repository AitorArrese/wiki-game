...
let find_path ?(max_depth = 3) ~origin ~destination ~how_to_fetch () =
  ...
  let rec rec_search positions_to_check depth =
    let solution =
      List.concat_map positions_to_check ~f:(fun (article, path) ->
        if String.equal article destination then [ path ] else [])
    in
    if List.is_empty solution
    then (
      let next_gen =
        List.concat_map positions_to_check ~f:(fun (article, path) ->
          ...
          then []
            ...
            |> List.map ~f:(fun article ->
              make_url article, path @ [ create_article_name article ])))
      in
      rec_search next_gen (depth - 1))
    else List.hd solution
  in
  rec_search [ origin, [ create_article_name origin ] ] max_depth
;;
